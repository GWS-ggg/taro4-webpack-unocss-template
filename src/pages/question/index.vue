  ...
  while (!shouldShowQuestion()) {
    ...
    if (currentQuestionIndex.value >= totalQuestionCount.value) {
      ...
      if (cacheData.value) {
        ...
        Taro.setStorageSync(`${surveyData.surveyId}_cacheData`, cacheData.value)
      }
      // 保存进度并关闭问卷 跳转页面
            ...
            >
              ...
              <view v-if="currentQuestion.type === 'multipleChoice'">
                ...
                <text
                  ...
                  class="text-18PX"
                >
                  {{ t('chooseLimit_head') }}{{ currentQuestion.maxSelection }}{{ t('chooseLimit_tail') }}
                </text>
                <view class="mt-5 w-full flex flex-col gap-2">
                  <label
                    v-for="option in currentQuestion.options"
              ...
              <view v-if="currentQuestion.type === 'numberText'">
                ...
                <text class="text-18PX">
                  {{ shownQuestionNumber }}.{{ currentQuestion.content }}{{ t('onlyNumber') }}
                </text>
                <view relative>
                  <input
                    type="number"
                    ...
                    class="mt-5 border-b-2PX border-[#f0f0f0] rounded border-b-solid bg-white p-2PX"
                    @focus="handleInputFocus"
                    @blur="handleInputBlur"
